...
for KEY in "${KEYS[@]}"; do
    kiichaind keys add $KEY --keyring-backend $KEYRING --algo $KEYALGO --home "$HOMEDIR"
done
...
for KEY in "${KEYS[@]}"; do
    kiichaind genesis add-genesis-account $KEY 100000000000000000000000000abera --keyring-backend $KEYRING --home "$HOMEDIR"
done
...
kiichaind genesis add-genesis-account cosmos1dgtgps0vxwt90hu6f3cceqypc5k664czp95ank 1000000000000000000abera,1000000000000000000asupply,1000000000000000000atoken,12345bAKT,1000000000000000000bATOM,24690bOSMO,1000000000000000000stake  --keyring-backend $KEYRING --home "$HOMEDIR"
kiichaind genesis add-genesis-account cosmos1h08vp7xt40nks7d0mlg47duyv54ewdxr0p0f44 100abera,100atoken,1000000000000000000stake --keyring-backend $KEYRING --home "$HOMEDIR"
kiichaind genesis add-genesis-account cosmos14nqnr8l8y2se3uu47qtyqehdfccfgwdlmshdpp 1000000000000000000abera --keyring-backend $KEYRING --home "$HOMEDIR"
...
kiichaind genesis gentx ${KEYS[0]} 1000000000000000000000abera --keyring-backend $KEYRING --chain-id $CHAINID --home "$HOMEDIR"
...
kiichaind start --pruning=nothing "$TRACE" --log_level $LOGLEVEL --api.enabled-unsafe-cors --api.enable --api.swagger --minimum-gas-prices=0.0001abera --home "$HOMEDIR"
